Feature: E.126.700 - The system shall support the ability to attach form status to the foot of the data entry form using Enhance Form Status module.
  ...
  Scenario: Enable external module in project
    Given I create a new project named "E.126.700" by clicking on "New Project" in the menu bar, selecting "Practice / Just for fun" from the dropdown, choosing file "fixtures/cdisc_files/E126700.xml", and clicking the "Create Project" button
    ...
    And I upload a "csv" format file located at "fixtures/import_files/E126700_Data_Import.csv", by clicking the button near "Select your CSV data file" to browse for the file, and clicking the button labeled "Upload File" to upload the file
    ...
    And I click on the button labeled "+" in the row labeled "Roles that can view the form status"
    And I select "DataEntry" on the dropdown field labeled "2. Roles that can view the form status"
    ...
    And I click on the link labeled "Arm Two"
    When I click on the link labeled "2-1"
    And I click the bubble to select a record for the "Data Types" longitudinal instrument on event "(#2)"
  ...
  Scenario: E.126.100 - Disable external module
    ...
    Then I should see a table header and row containing the following values in the logging table:
      ...
      | mm/dd/yyyy hh:mm | test_admin | Disable external module "enhance_form_status_v1.1.1" for project                  |                                                                                           |
      | mm/dd/yyyy hh:mm | test_admin | Modify configuration for external module "enhance_form_status_v1.1.1" for project | show-form-status-inline                                                                   |
      | mm/dd/yyyy hh:mm | test_admin | Modify configuration for external module "enhance_form_status_v1.1.1" for project | show-form-status-inline, text-representing-in-progress, ignore-for-form-status-check      |
      | mm/dd/yyyy hh:mm | test_admin | Modify configuration for external module "enhance_form_status_v1.1.1" for project | reserved-hide-from-non-admins-in-project-list, user-roles-can-update, user-roles-can-view |
      | mm/dd/yyyy hh:mm | test_admin | Enable external module "enhance_form_status_v1.1.1" for project                   |                                                                                           |
    ...
    And I should NOT see "E.126.700"
    ...
    Then I should see a table header and row containing the following values in a table:
      | Time             | User       | Event                                                                             |
      | mm/dd/yyyy hh:mm | test_admin | Disable external module "enhance_form_status_v1.1.1" for system                   |
      | mm/dd/yyyy hh:mm | test_admin | Disable external module "enhance_form_status_v1.1.1" for project                  |
      | mm/dd/yyyy hh:mm | test_admin | Modify configuration for external module "enhance_form_status_v1.1.1" for project |
      | mm/dd/yyyy hh:mm | test_admin | Enable external module "enhance_form_status_v1.1.1" for project                   |
      | mm/dd/yyyy hh:mm | test_admin | Enable external module "enhance_form_status_v1.1.1" for system                    |
